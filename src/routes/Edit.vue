<template>
    ...
    >
        ...
        <el-form-item>
            <el-button type="success" @click="onSubmit(ruleFormRef)">Редактировать</el-button>
        </el-form-item>
    </el-form>
</template>
...
const form = reactive<IItem>(store.state.items.find(item => item.id === id) || null!);
...
const onSubmit = async (formEl: FormInstance | undefined) => {
    ...
    await formEl.validate((valid, fields) => {
        if (!valid) return;
        store.commit("update", {id, item: ({...form} as IItem)})
    })
    }
</script>
